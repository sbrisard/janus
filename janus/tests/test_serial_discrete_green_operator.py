...
class AbstractTestDiscreteGreenOperator:
    ...
    def pytest_generate_tests(self, metafunc):
        if metafunc.function.__name__ == 'test_init_invalid_params':
            ...
            g2 = self.greend(greenop.create(Material(MU, NU, 2)),
                             (8, 16), 1.0)
            g3 = self.greend(greenop.create(Material(MU, NU, 3)),
                             (8, 16, 32), 1.0)
            ...
            g = self.greend(greenop.create(Material(MU, NU, 2)),
                            (8, 16), 1.0)
            i0, i1, i2 = g.ishape
            o0, o1, o2 = g.oshape
            ...
            g = self.greend(greenop.create(Material(MU, NU, 3)),
                            (8, 16, 32), 1.0)
    ...
    def test_to_memoryview(self, n, flag):
        dim = len(n)
        greenc = greenop.create(Material(MU, NU, dim))
    ...
    def test_apply_by_freq(self, n, x, flag):
        ...
        """
        dim = len(n)
        green = self.greend(greenop.create(Material(MU, NU, dim)), n, 1.)
        for b in multi_indices(n):
    ...
    def test_apply(self, path_to_ref, rtol, flag):
        ...
        green = self.greend(greenop.create(Material(MU, NU, len(n))),
